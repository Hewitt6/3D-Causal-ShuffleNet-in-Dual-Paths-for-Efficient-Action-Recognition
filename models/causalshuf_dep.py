...
class TwoPathwayShuffleNet(nn.Module):
    def __init__(self,
        ...
        self.alpha = int(24 * width_mult)
        
        self.slow_buf = torch.zeros(64, self.alpha, 1, 56, 56).cuda()
        self.fast_buf = torch.zeros(64, self.alpha, 1, 56, 56).cuda()

        self.register_buffer('slow_buf', self.slow_buf)
        self.register_buffer('fast_buf', self.fast_buf)
        
        self.slow_conv = conv_bn(3, self.alpha, stride=(1,2,2))
        self.slow_maxpool = nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1))

        self.fast_avgpool = nn.AvgPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1))
        self.fast_conv = conv_bn(3, self.alpha, kernel_size=1, stride=1, padding=0)
        self.fast_maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)

        self.slow_shufflenet = ShuffleNet(groups=self.groups, width_mult=width_mult, num_classes=self.num_classes)
        self.fast_shufflenet = ShuffleNet(groups=self.groups, width_mult=width_mult, num_classes=self.num_classes)

        self.classifier = nn.Sequential(
                            nn.Dropout(0.2),
                            nn.Linear(int(960 * width_mult * 2), self.num_classes)
                            )
        
    def forward(self, x):
        # input shape (batch_size, 3, 15, 112, 112)
        slow_x = x[:, :, :14:2, :, :]
