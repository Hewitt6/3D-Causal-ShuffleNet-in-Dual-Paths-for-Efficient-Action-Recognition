...
class TwoPathwayShuffleNet(nn.Module):
    def __init__(self,
        ...
        self.slow_maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)

        self.fast_avgpool = nn.AvgPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1))
        self.fast_conv = conv_bn(3, self.alpha, stride=(2,2,2))
        self.fast_maxpool = nn.MaxPool3d(kernel_size=(3, 1, 1), stride=(2, 1, 1), padding=(1, 0, 0))
        ...
        self.classifier = nn.Sequential(
                            nn.Dropout(0.2),
                            nn.Linear(int(480 * width_mult * 2), self.num_classes)
                            )
        
    def forward(self, x):
        # input shape (batch_size, 3, 32, 112, 112)
        slow_x = x[:, :, ::2, :, :]
